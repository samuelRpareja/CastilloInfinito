...
public class MeleeMageController : MonoBehaviour, IEnemyAttack
{
    ...
    [SerializeField] private float recover = 0.7f; // Tiempo despu�s del golpe

    [Header("Referencias")]
    [SerializeField] private Animator animator; // Arrastra el Animator aqu�
    [SerializeField] private HitboxDamager hitbox; // Arrastra el hitbox del arma/mano aqu�
    ...
    public void DoAttack()
    {
        ...
        // �Aqu� est� la magia! Le decimos al Animator que active el trigger del ataque
        animator.SetTrigger("doAttack");

        // Activamos el hitbox durante la animaci�n (usando un Invoke o Animation Events)
        // Por ejemplo, activarlo despu�s del 'windup'
        Invoke(nameof(EnableHitbox), windup);
        Invoke(nameof(DisableHitbox), windup + 0.2f); // Desactivarlo 0.2s despu�s
    }
    ...
    private void EnableHitbox()
    {
        if (hitbox != null)
        {
            hitbox.SetDamage(damage); // Le pasamos el da�o al hitbox
    ...
    {
        return windup + recover; // Duración total del ataque
    }
}