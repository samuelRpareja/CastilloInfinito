...
public class AoEAttack : MonoBehaviour, IEnemyAttack
{
    ...
    [SerializeField] private float windupTime = 0.7f; // telegraf�a
    ...
    private IEnumerator CastCo()
    {
        ...
        // Si quieres que el c�rculo "siga" al target durante la telegraf�a
        ITarget tgt = TargetRegistry.Instance?.CurrentTarget;
        float t = 0f;

        // TODO: Instanciar VFX telegr�fico (c�rculo en el suelo)
        ...
        foreach (var h in hits)
        {
            if (h.TryGetComponent<IDamageable>(out var dmg))
            {
                ...
                // Vamp�rico (si existe en el caster)
    ...
    {
        return windupTime; // La duración del ataque es el tiempo de windup
    }
